...
release_raw <- read_xlsx(here::here("data-raw", "lower_feather_release.xlsx")) |> #no markLifeStage, appliedMarkCode
  ...
  #For the 2025 trapping season all fish used for Etrials were from the Feather River Fish Hatchery
  mutate(sourceOfFishSite = case_when(year(releaseTime) == 2025 ~ "Feather River Fish Hatchery",
                                      TRUE ~ NA)) |>
  relocate(sourceOfFishSite, .after = markedFishOrigin) |>
  glimpse()
